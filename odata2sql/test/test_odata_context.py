...
def test_adjust_next_url_not_needed(context_curia_vista):
    assert context_curia_vista.adjust_next_url(
...
def test_adjust_next_url(client_curia_vista):
    ctx = Context(client_curia_vista, SettingsBuilder().url('http://localhost:8080/odata.svc').build())
...
def test_get_dependency_with_multiplicity(context_curia_vista):
    transcript = context_curia_vista.client.schema.entity_type('Transcript')
    results = set(context_curia_vista.get_dependencies_with_multiplicity(transcript))
    assert results == {
        Multiplicity(transcript, '*', context_curia_vista.client.schema.entity_type('MemberCouncil'), '1'),
        Multiplicity(transcript, '*', context_curia_vista.client.schema.entity_type('Subject'), '1'),
        Multiplicity(transcript, '*', context_curia_vista.client.schema.entity_type('Business'), '0..1')
    }
...
def test_can_be_synced_by(context_curia_vista, dependant_name, principal_name, expected, comment):
    dependant = context_curia_vista.client.schema.entity_type(dependant_name)
    principal = context_curia_vista.client.schema.entity_type(principal_name)
    assert expected == context_curia_vista.can_be_synced_by(dependant, principal), comment
...
def test_context_include(client_curia_vista, config, entity_type_names_to_include, comment):
    ctx = Context(client_curia_vista, SettingsBuilder(SERVICE_URL).sync_config(config).build())
...
def test_context_skip(client_curia_vista, config, entity_type_names_to_skip, comment):
    ctx = Context(client_curia_vista, SettingsBuilder(SERVICE_URL).sync_config(config).build())
    assert {et.name for et in ctx.skip} == entity_type_names_to_skip, comment


def test_setting_invalid_entity_type_name(client_curia_vista):
    with pytest.raises(ValueError) as e:
        Context(client_curia_vista, SettingsBuilder(SERVICE_URL).sync_config({'entities': {'NotValid': {}}}).build())

    assert str(e.value) == 'Invalid entity type name: "NotValid"'


def test_odata_sync_by_fk_invalid_principal(client_curia_vista):
    with pytest.raises(ValueError) as e:
        Context(client_curia_vista,
...
def test_odata_faulty_fk_relation(client_curia_vista, config, dependant, principal, comment):
    with pytest.raises(ValueError) as e:
        Context(client_curia_vista, SettingsBuilder(SERVICE_URL).sync_config(config).build())

    assert str(e.value) == f'"{dependant}" is not a dependency of "{principal}"', comment


def test_odata_faulty_fk_multiplicity(client_curia_vista):
    with pytest.raises(ValueError) as e:
        Context(client_curia_vista,
...
def test_selected_properties(client_curia_vista, config, entity_type_name, properties, comment):
    ctx = Context(client_curia_vista, SettingsBuilder(SERVICE_URL).sync_config(config).build())
